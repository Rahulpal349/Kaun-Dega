...
export function computeBalances(members, expenses, settlements) {
  ...
  (expenses || []).forEach((e) => {
    let shares = (e.expense_shares || e.shares || []).map((s) => ({ ...s }));
    const shareTotal = shares.reduce((s, x) => s + Number(x.share_amount || x.amount || 0), 0);
    const paidBy = e.paid_by || e.paidBy;

    // In a 2-person / 1-on-1 Khatabook, if only 1 share exists and it was mistakenly assigned to the payer
    // (e.g. entry added on behalf of the other person), the debtor is the other member.
    if (memberIdSet.size === 2 && paidBy && shares.length === 1) {
      const shareUid = shares[0].user_id || shares[0].userId;
      if (shareUid === paidBy) {
        const otherId = Array.from(memberIdSet).find((id) => id !== paidBy);
        if (otherId) {
          shares[0].user_id = otherId;
          shares[0].userId = otherId;
        }
      }
    }
...
export function buildWhatsappText(group, moves) {
  ...
  const inviteUrl = groupId ? `${typeof window !== 'undefined' ? window.location.origin : 'https://kaun-dega.vercel.app'}/join/${groupId}` : '';
  ...
  const buffer = [`*${name}* — Hisaab Kitab:`, ...lines, ''];
  if (inviteUrl) {
    buffer.push(`🔗 Join Group Link: ${inviteUrl}`);
    buffer.push(`🔑 Invite Code: ${groupId}`);
    buffer.push('');
  }
  buffer.push('Settle up on Kaun Dega:');
  return buffer.join('\n');
}
