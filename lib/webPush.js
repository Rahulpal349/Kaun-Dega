...
export async function requestAndRegisterPushNotifications(userId) {
  ...
  try {
    ...
    // Get active service worker registration to prevent duplicate scope conflicts
    let swRegistration;
    if ('serviceWorker' in navigator) {
      ...
      if (!swRegistration) {
        try {
          ...
          swRegistration = await navigator.serviceWorker.register('/firebase-messaging-sw.js', { scope: '/' }).catch(() => undefined);
        }
      }
    }

    const vapidKey = process.env.NEXT_PUBLIC_FIREBASE_VAPID_KEY || 'BEpc8NPkH2AZ1FuRtRg61adOp1eRaxxDZaTkgMzh8laKW7WDIr3RS8_yRQbM1jtPxui4EreUCp4pyyNWd_TcQtA';
    const tokenOptions = {
      ...(vapidKey ? { vapidKey } : {}),
