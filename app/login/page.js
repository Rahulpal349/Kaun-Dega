...
import { auth, googleProvider, signInWithEmailAndPassword, signInWithPopup, onAuthStateChanged } from '../../lib/firebase';
...
export default function LoginPage() {
  ...
  // Redirect if already logged in with Firebase
  useEffect(() => {
    ...
    <main className="min-h-screen bg-green-50 flex items-center justify-center px-6 relative overflow-hidden">
      ...
      <div className="bg-white rounded-[2rem] w-full max-w-sm p-8 shadow-2xl shadow-gray-200/50 border border-gray-100 relative z-10 mt-12">
        ...
        <p className="text-sm text-ink/60 mt-8 text-center font-medium">
          New here?{' '}
          <Link href="/signup" className="text-primary font-bold hover:underline underline-offset-4 decoration-primary/30">
            Create an account
          </Link>
        </p>
      </div>
