...
export default function NotificationPrompt() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      // If already granted, ensure token is registered silently
      if (perm === 'granted') {
        requestAndRegisterPushNotifications(user.uid).catch(() => {});
        return;
      }

      // If default (not yet chosen), display prompt
      if (perm === 'default') {
        if (sessionStorage.getItem('kd_notif_dismissed') === 'true') {
          return;
        }
        // Delay showing by 1s
  ...
  async function handleEnable() {
    ...
    try {
      const token = await requestAndRegisterPushNotifications(userId);
  ...
  return (
    <div className="fixed bottom-6 left-4 right-4 sm:left-auto sm:right-6 sm:max-w-md z-[999] animate-slide-up">
      <div className="bg-white/95 backdrop-blur-md rounded-2xl p-4 sm:p-5 shadow-2xl border-2 border-[#145C4B]/20 flex flex-col gap-3">
